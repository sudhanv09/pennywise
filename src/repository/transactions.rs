...
pub fn get_all(db: &DbConnection) -> Result<Vec<Transactions>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, title, amount, tx_date, tx_time, tx_type, category, account, description, goal_id, loan_id FROM transactions",
    )?;
...
pub fn get_by_id(db: &DbConnection, id: i32) -> Result<Transactions> {
    let conn = db.lock().unwrap();
    conn.query_row(
        "SELECT id, title, amount, tx_date, tx_time, tx_type, category, account, description, goal_id, loan_id FROM transactions WHERE id = ?1",
        [id],
        |row| {
            let date_str: String = row.get(3)?;
            let time_str: String = row.get(4)?;
...
pub fn insert(db: &DbConnection, tx: &Transactions) -> Result<i64> {
    let conn = db.lock().unwrap();
    conn.execute(
        "INSERT INTO transactions (title, amount, tx_date, tx_time, tx_type, category, account, description, goal_id, loan_id)
...
pub fn update(db: &DbConnection, tx: &Transactions) -> Result<()> {
    ...
    conn.execute(
        "UPDATE transactions SET title = ?1, amount = ?2, tx_date = ?3, tx_time = ?4,
         ...
         goal_id = ?9, loan_id = ?10 WHERE id = ?11",
        params![
            tx.title,
            tx.amount as f64,
            tx.tx_date.format("%Y-%m-%d").to_string(),
...
pub fn get_by_month(db: &DbConnection, month: u32, year: i32) -> Result<Vec<Transactions>> {
    let conn = db.lock().unwrap();
    let mut stmt = conn.prepare(
        "SELECT id, title, amount, tx_date, tx_time, tx_type, category, account, description, goal_id, loan_id
         FROM transactions
