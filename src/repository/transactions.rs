...
pub fn get_all(db: &DbConnection) -> Result<Vec<Transactions>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, title, amount, tx_date, tx_time, tx_type, category, account, description FROM transactions",
    )?;
    let rows = stmt.query_map([], |row| {
        ...
        Ok(Transactions {
            ...
            tx_date: NaiveDate::parse_from_str(&date_str, "%Y-%m-%d")
                .unwrap_or_default(),
            tx_time: NaiveTime::parse_from_str(&time_str, "%H:%M:%S")
                .unwrap_or_default(),
...
pub fn get_by_id(db: &DbConnection, id: i32) -> Result<Transactions> {
    let conn = db.lock().unwrap();
    conn.query_row(
        "SELECT id, title, amount, tx_date, tx_time, tx_type, category, account, description FROM transactions WHERE id = ?1",
        [id],
        |row| {
            let date_str: String = row.get(3)?;
            let time_str: String = row.get(4)?;
...
pub fn insert(db: &DbConnection, tx: &Transactions) -> Result<i64> {
    let conn = db.lock().unwrap();
    conn.execute(
        "INSERT INTO transactions (title, amount, tx_date, tx_time, tx_type, category, account, description)
...
pub fn update(db: &DbConnection, tx: &Transactions) -> Result<()> {
    ...
    conn.execute(
        "UPDATE transactions SET title = ?1, amount = ?2, tx_date = ?3, tx_time = ?4,
         tx_type = ?5, category = ?6, account = ?7, description = ?8 WHERE id = ?9",
        params![
            tx.title,
            tx.amount as f64,
            tx.tx_date.format("%Y-%m-%d").to_string(),
